...
void ButtonIOTask(void* pdata)
{
    ...
    while (1)
    {
        ...
        if (buttons & CRUISE_CONTROL_FLAG) {
            ...
            OSMboxPost(Mbox_CruiseControl, (void*)on);
        }
        else OSMboxPost(Mbox_CruiseControl, (void*)off);
        ...
        if (buttons & BRAKE_PEDAL_FLAG) {
            ...
            OSMboxPost(Mbox_Brake1, (void*)on);
        }
        else {
            OSMboxPost(Mbox_Brake, (void*)off);
            OSMboxPost(Mbox_Brake1, (void*)off);
        }

        if (buttons & GAS_PEDAL_FLAG) {
            printf("[BUTTON IO TASK] Gas pedal pressed.\n");
            led_green = led_green | LED_GREEN_6;
            OSMboxPost(Mbox_GasPedal, (void*) on);
        } 
        else OSMboxPost(Mbox_GasPedal, (void*) off);
    ...
    while (1) {
        ...
        if (err == OS_NO_ERR) {
            if (semData.OSCnt < 1) {
                ...
                printf("[WATCHDOG TASK] OVERLOAD DETECTED (QSQSize = %d).\n", semData.OSCnt);
            }
        } else {
...
void printCreateTaskError(INT8U err) {
    switch (err) {
        case OS_NO_ERR:
            ...
            break;
            //        case OS_PRIO_EXISTS:
            //            printf("OS_PRIO_EXISTS: The requested priority already exist\n");
            //            break;
